...
<head>
    ...
    <title>{{ $selectedCategory ? $selectedCategory->name . ' - ' : '' }}Products - ShopSphere</title>
    <meta name="description" content="Browse our wide selection of products across electronics, fashion, home essentials, wellness and more on ShopSphere.">
...
<body class="bg-gray-50 text-gray-800 font-['Inter',sans-serif] min-h-screen flex flex-col">
                    ...
                    @else
                        ...
                        <div class="relative" id="profileDropdownWrapper">
                            <button type="button"
                                ...
                                <div class="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-600 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-sm ring-2 ring-white">
                            ...
                            <div id="profileDropdownMenu"
                                ...
                                @else
                                    <a href="{{ url('/dashboard') }}" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 font-medium transition">
        ...
        <div class="md:hidden px-4 pb-3">
            <form action="{{ route('products') }}" method="GET" class="flex">
                ...
                <input
                    ...
                    class="w-full h-10 px-4 border border-gray-300 rounded-l-xl focus:outline-none focus:border-indigo-500 text-sm"
                >
                <button
                    type="submit"
                    aria-label="Search"
                    class="h-10 px-4 bg-indigo-600 text-white rounded-r-xl flex items-center justify-center"
                >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
    ...
    {{-- =========================================================
         PAGE HERO BANNER
    ========================================================== --}}
    <section class="bg-gradient-to-r from-indigo-700 via-indigo-600 to-purple-700 text-white py-10 shadow-inner">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <!-- Breadcrumbs -->
                    <nav class="flex items-center gap-2 text-xs text-indigo-200 mb-2 font-medium">
                    ...
                    <p class="text-indigo-100 text-sm md:text-base mt-1.5 max-w-2xl">
                        @if($selectedCategory && $selectedCategory->description)
                            {{ $selectedCategory->description }}
                        @else
                            Discover top-quality verified products from registered sellers across multiple categories.
                        @endif
                    </p>
                </div>

                <div class="flex items-center gap-3 shrink-0">
                    <span class="px-4 py-2 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 text-xs font-semibold tracking-wide uppercase">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-200/80">
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-gray-100 mb-5">
                        ...
                        @if(request('category') || request('search') || request('min_price') || request('max_price') || request('in_stock'))
                            <a href="{{ route('products') }}" class="text-xs text-indigo-600 hover:text-indigo-800 font-semibold transition">
                                Reset All
                            </a>
                        @endif
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <a href="{{ route('products', array_filter(request()->except(['category', 'page']))) }}"
                               class="flex items-center justify-between px-3 py-2 rounded-xl text-sm font-medium transition {{ !request('category') ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
                                <span>All Categories</span>
                                <span class="text-xs px-2 py-0.5 rounded-full {{ !request('category') ? 'bg-indigo-200/60 text-indigo-800' : 'bg-gray-100 text-gray-500' }}">
                                    {{ $totalActiveProducts }}
                                </span>
                            </a>
                            ...
                            @foreach($categories as $cat)
                                ...
                                <a href="{{ route('products', array_merge(request()->except(['page']), ['category' => $cat->slug])) }}"
                                   class="flex items-center justify-between px-3 py-2 rounded-xl text-sm font-medium transition {{ $isActive ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
                                    <span class="truncate">{{ $cat->name }}</span>
                                    <span class="text-xs px-2 py-0.5 rounded-full {{ $isActive ? 'bg-indigo-200/60 text-indigo-800' : 'bg-gray-100 text-gray-500' }}">
                                        {{ $cat->products_count }}
                                    </span>
                                </a>
                            @endforeach
                    ...
                    <div class="pt-5 border-t border-gray-100">
                        ...
                        <form action="{{ route('products') }}" method="GET" class="space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input
                                        ...
                                        class="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-indigo-500"
                                    >
                                </div>
                                <div>
                            ...
                            <button
                                type="submit"
                                class="w-full py-2 bg-indigo-50 text-indigo-600 font-semibold text-xs rounded-lg hover:bg-indigo-100 transition"
                            >
                                Apply Price
                            </button>
                        </form>
            ...
            <section class="lg:col-span-3">
                ...
                <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-200/80 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="text-sm font-semibold text-gray-700">
                            Showing <span class="text-indigo-600 font-bold">{{ $products->count() }}</span> of {{ $products->total() }} results
                        </span>
                        ...
                        @if(request('category') && $selectedCategory)
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200/60">
                                ...
                                <a href="{{ route('products', array_filter(request()->except(['category', 'page']))) }}" class="hover:text-indigo-900">&times;</a>
                            </span>
                        @endif
                        ...
                        @if(request('search'))
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200/60">
                                ...
                                <a href="{{ route('products', array_filter(request()->except(['search', 'page']))) }}" class="hover:text-indigo-900">&times;</a>
                            </span>
                        @endif
                        ...
                        @if(request('min_price') || request('max_price'))
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200/60">
                                ...
                                <a href="{{ route('products', array_filter(request()->except(['min_price', 'max_price', 'page']))) }}" class="hover:text-indigo-900">&times;</a>
                            </span>
                        @endif
                    </div>
                    ...
                    <form action="{{ route('products') }}" method="GET" class="flex items-center gap-2">
                        ...
                        <select
                            ...
                            class="text-sm font-medium border border-gray-300 rounded-xl px-3 py-1.5 bg-white text-gray-700 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                        >
                                ...
                                <div class="relative h-60 bg-gray-50 flex items-center justify-center overflow-hidden">
                                    ...
                                    @if($product->featured_image)
                                        <img
                                            ...
                                            onerror="this.onerror=null; this.src='https://placehold.co/400x400/f3f4f6/6366f1?text={{ urlencode($product->name) }}';"
                                        >
                                    @else
                                        <!-- Fallback Vector Icon Container -->
                                        <div class="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-indigo-50/60 to-purple-50/40 text-indigo-400">
                                            ...
                                            <span class="text-xs text-gray-400 mt-2 font-medium">{{ $product->category->name ?? 'Product' }}</span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="absolute top-3 left-3 flex flex-col gap-1.5">
                                        @if($product->is_featured)
                                            <span class="px-2.5 py-1 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[10px] font-bold rounded-lg shadow-sm">
                                                FEATURED
                                            </span>
                                        @endif

                                        @if($product->discount_percentage > 0)
                                            <span class="px-2.5 py-1 bg-emerald-500 text-white text-[10px] font-bold rounded-lg shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        @if($product->category)
                                            <a href="{{ route('products', ['category' => $product->category->slug]) }}"
                                               class="text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition tracking-wide uppercase">
                                                {{ $product->category->name }}
                                            </a>
                                        @endif

                                        <!-- Title -->
                                        <h3 class="font-bold text-gray-900 text-base mt-1 line-clamp-2 group-hover:text-indigo-600 transition">
                                            {{ $product->name }}
                                        </h3>
                                    ...
                                    <div class="mt-4 pt-3 border-t border-gray-100">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <button
                                                ...
                                                class="flex-1 py-2.5 px-4 bg-indigo-600 text-white font-semibold text-xs rounded-xl hover:bg-indigo-700 transition flex items-center justify-center gap-2 shadow-sm"
                                            >
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                                                </svg>
                ...
                @else
                    <!-- Empty State -->
                    <div class="bg-white rounded-3xl p-12 text-center border border-gray-200/80 shadow-sm max-w-lg mx-auto my-6">
                        <div class="w-20 h-20 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-500 mx-auto mb-5">
                        ...
                        <p class="text-sm text-gray-500 mb-6 max-w-md mx-auto leading-relaxed">
                            We couldn't find any products matching your selected category or search filters. Try adjusting your criteria or reset all filters.
                        </p>

                        <div class="flex justify-center gap-3">
                            <a href="{{ route('products') }}"
                               class="px-5 py-2.5 bg-indigo-600 text-white font-semibold text-sm rounded-xl hover:bg-indigo-700 transition">
                                Clear All Filters
                            </a>
                            <a href="{{ route('home') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-10">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-xl font-bold text-white">ShopSphere</h3>
                    </div>
                    <p class="mt-4 text-xs leading-relaxed">
                        Your trusted multi-seller marketplace with quality products and transparent referral commissions.
                    </p>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-2.5 text-xs">
                        ...
                        <a href="{{ route('products') }}" class="block hover:text-white transition">All Products</a>
                        <a href="{{ route('about') }}" class="block hover:text-white transition">About Us</a>
                        <a href="{{ route('contact') }}" class="block hover:text-white transition">Contact Us</a>
                    </div>
                </div>

                <!-- Categories -->
                <div>
                    <h4 class="text-white text-sm font-semibold mb-4">Top Categories</h4>
                    <div class="space-y-2.5 text-xs">
